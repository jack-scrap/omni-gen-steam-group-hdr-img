...
void scn::init(unsigned int stage, unsigned int lvl) {
	...
	GLfloat st[3 * 2 * 3 * 2 * 2];
	for (int i = 0; i < 3 * 2 * 3 * 2 * 2; i += 3 * 2 * 2) {
		st[i] = 0;
		st[i + 1] = 0;
		st[i + 2] = 1;
		st[i + 3] = 0;
	...
	GLushort idc[sizeof vtc / sizeof *vtc];
	for (int i = 0; i < sizeof idc / sizeof *idc; i++) {
		idc[i] = i;
	}

	Obj* obj = objMk(vtc, sizeof vtc / sizeof *vtc, st, sizeof st / sizeof *st, idc, sizeof idc / sizeof *idc, "obj", "tex", true);
