...
Truck* truckMk(Array* init, glm::vec3 loc, glm::vec3 rot) {
	...
	for (int z = 0; z < 2; z++) {
		for (int x = 0; x < init->_x + 1; x++) {
			...
			child[4 + i] = objMk("wheel", "obj", "dir", false, rim, sizeof rim / sizeof *rim, overhead + glm::vec3(x * -(layout::idx[Z] + (layout::margin * 2) + (layout::stroke * 2) + (layout::margin * 2)), -1, (z ? 1 : -1) * ((layout::idx[X] / 2) + layout::margin + layout::margin + layout::stroke + layout::margin + layout::margin) - (0.8 / 2)), glm::vec3(0.0, z * M_PI, 0.0));
