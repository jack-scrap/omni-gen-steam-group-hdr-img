...
void main() {
	...
	for	(int z = 0; z < 2; z++) {
		for	(int y = 0; y < 2; y++) {
			for	(int x = 0; x < 2; x++) {
				vec3 pos = vec3(bound[0][x] - ((bool(x) ? 1 : -1) * fac), (bool(y) ? 1 : -1) * pad * 2, bound[1][z]);
	...
	for	(int x = 0; x < 2; x++) {
		for (int y = 0; y < 2; y++) {
			for (int z = 0; z < 2; z++) {
				vec3 pos = vec3(bound[0][x], (bool(y) ? 1 : -1) * pad * 2, bound[1][z] - ((bool(z) ? 1 : -1) * fac));
	...
	// corners
	for (int i = 0; i < 2; i++) {
		vec3 corner = vec3(bound[0][i], 0.0, bound[1][i]) - vec3((bool(i) ? 1 : -1) * fac, 0.0, (bool(i) ? 1 : -1) * fac);
