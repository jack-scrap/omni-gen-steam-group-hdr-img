...
.PHONY: all
all: mk_build mk_script mk_o omni_gen_steam_community_group_hdr_img ro
...
omni_gen_steam_community_group_hdr_img: $(OBJ_STATIC) $(OBJ_DYNA) $(HDR)
...
.PHONY: install
install:
	sudo mv omni_gen_steam_community_group_hdr_img $(DESTDIR) ;
...
.PHONY: clean
clean:
	rm $(BUILDDIR)/*.o $(BUILDDIR)/*.so omni_gen_steam_community_group_hdr_img
